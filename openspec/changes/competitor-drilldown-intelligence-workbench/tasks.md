...
- [x] 1.1 Replace the global tabs-first default with a competitor-list-first workbench entry.
- [x] 1.2 Add selected-competitor state and derive detail data from the selected competitor.
- [x] 1.3 Ensure switching competitors clears or refreshes stale target, task, report, and draft state.
- [ ] 1.4 Keep global report/task access secondary and label it as cross-competitor.

## 2. Competitor Detail Workspace

- [x] 2.1 Build the selected competitor detail layout with overview, targets, collection, reports, and tasks sections.
- [x] 2.2 Move target recommendation into the selected competitor detail.
- [x] 2.3 Rename candidate target UI copy to `待确认监控页` or `待加入监控页`.
...
- [x] 3.1 Scope report lists and report detail entry to the selected competitor.
- [x] 3.2 Scope task run lists to the selected competitor by target relation or backend query parameter.
- [x] 3.3 Add minimal report/task `competitorId` query support only if frontend scoping is unreliable.
- [x] 3.4 Preserve evidence-first report detail, feedback, and knowledge writeback actions in the competitor context.
